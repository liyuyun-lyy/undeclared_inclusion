...
bazel clean
bazel build -s :hello --config=clang --config=debug --config=remote --noremote_accept_cached --remote_executor=grpc://localhost:8980 --experimental_remote_grpc_log=$HOME/remote.log 
...
bazel build -s :hello --config=clang --config=debug --config=remote --remote_accept_cached --remote_executor=grpc://localhost:8980 --experimental_remote_grpc_log=$HOME/remote.log
